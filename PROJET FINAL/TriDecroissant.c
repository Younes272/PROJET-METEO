...
typedef struct Noeud {
    ...
    char ligne[1500]; // Ligne entière du fichier CSV
    int height;
    struct Noeud* left;
    struct Noeud* right;
}Noeud;
...
void decroissant(Noeud *node, FILE* output) {
    if (node != NULL) {
        decroissant(node->right,output);
        fprintf(output, "%s\n", node->ligne);
        decroissant(node->left,output);	
        }
}
